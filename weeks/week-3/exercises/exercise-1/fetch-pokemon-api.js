...
const fetchPokemons = () => {
  ...
  fetch(url)
    ...
    .then((data) => {
      console.log(data.results); // 2.a
      console.log(data.results[0].name); // 2.b
      data.results.forEach((pokemon) => console.log(pokemon.name)); //2.c
      pokemonUrl = data.results[24].url;
      ...
      const fetchPikachuData = () => {
        ...
        fetch(pokemonUrl)
          ...
          .then((d) => {
            console.log(d);
            image.src = d.sprites.front_default; //5
            name.innerHTML = d.name;
            element.innerHTML = d.stats[1].stat.name;
            weight.innerHTML = d.weight;
            height.innerHTML = d.height;
            types.innerHTML = d.types[0].type.name;
      ...
      fetchPikachuData();
      console.log(`Pika's URL: ${pokemonUrl}`);
    })
    .catch((err) => console.log(err));
};
